...
function debug.dumpTable(buffer, t, pretty, depth, stack)
  if stack[t] then
    table.insert(buffer, "(")
  ...
  for i, element in ipairs(t) do
    ...
    debug.dumpValue(buffer, element, pretty, depth + 1, stack)
  ...
  for name, element in pairs(t) do
    ...
    if not isArrayIndex and not isMetamethodName then
      ...
      if type(name) == "string" and
        ...
        table.insert(buffer, "[")
        debug.dumpValue(buffer, name, pretty, depth + 1, stack)
        table.insert(buffer, "]")
      end
...
function debug.dumpValue(buffer, value, pretty, depth, stack)
  if type(value) == "string" then
    table.insert(buffer, string.format("%q", value))
  elseif type(value) == "table" then
    debug.dumpTable(buffer, value, pretty, depth, stack)
  else
    table.insert(buffer, tostring(value))
  end
end

function debug.dump(value, format)
  format = format or "compact"
  assert(format == "compact" or format == "pretty")
  local buffer = {}
  debug.dumpValue(buffer, value, format == "pretty", 0, {})
